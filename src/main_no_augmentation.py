...
class_names = ['normal', 'abnormal_hb', 'history_MI', 'MI']
...
def visualize_samples(n_samples=3):
    ...
    plt.tight_layout()
    plt.savefig('./results/original_samples.png')
    plt.show()
...
def show_class_distribution():
    ...
    plt.savefig('./results/original_class_dist.png')
    plt.show()
...
def train_kfold(df, n_splits=5, num_epochs=15):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(df.img, df.label)):
        ...
        val_loader = DataLoader(test_dataset, batch_size=32, shuffle=True, num_workers=4)
...
def plot_metrics(metrics):
    ...
    plt.tight_layout()
    plt.savefig('./results/original_val_loss_acc.png')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('./results/original_train_val_loss_accuracy.png')
    plt.show()
...
# Save the best model
torch.save(model.state_dict(), 'model_no_augmentation.pth')
