...
# Define class names (without Covid)
class_names = ['normal_nst', 'abnormal_hb_nst', 'history_MI_nst', 'MI_nst']

# Function to get paths for all images in each class
def get_image_paths():
    image_paths = {}
...
def visualize_samples(n_samples=3):
    ...
    plt.tight_layout()
    plt.savefig('./results/nst_samples.png')
    plt.show()

# Display class distribution
def show_class_distribution():
    counts = {cls: len(paths) for cls, paths in all_image_paths.items()}
    ...
    plt.savefig('./results/nst_class_dist.png')
    plt.show()
...
def plot_metrics(metrics):
    ...
    plt.tight_layout()
    plt.savefig('./results/nst_val_loss_acc.png')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('./results/nst_train_val_loss_accuracy.png')
    plt.show()
...
def display_results(results):
    ...
    # Aggregate and average metrics across folds
    # Class mappings
    class_names = {
        '0': 'Normal',
        '1': 'MI_heat',
    ...
    for class_name in ['Normal', 'MI_heat', 'MI_recovered', 'MI', 'macro', 'weighted']:
...
# Save the best model
torch.save(model.state_dict(), 'model_nst_augmentation.pth')